"""One session, drawn: the chart core both the Lab and the journal build on.

This is ``sim_charts._session_frame`` with the strategy taken out of it. What it
needed from a run's config was only ever three things — which contract, how many
ticks to a bar, and whether the engine read the night — and none of those are
facts about a *strategy*. Everything else here (the anchored VWAPs, the two
developing profiles, the IB, the EMA/RSI/ATR, the footprint and the CVD with its
divergences) is a fact about a **session**, and a session is a session whether a
backtest traded it or you did.

That mattered once the journal's charts moved off Databento's 1-minute bars and
onto the same tick cache the sim reads (2026-08-08). Before, the two sides could
not have shared this even in principle: the journal had one volume number per
minute, so it could only smear a bar's volume across its range, and half the
overlays below (footprint, CVD, exact profiles) need the tape itself. Now both
sides start from the same ticks, and the difference between a Lab chart and a
journal chart is only what gets drawn *on top* — a run's trades, or yours.

TWO CALLERS, ONE RULE ABOUT MONEY. ``api.sim_charts`` serves runs and may buy a
session it hasn't got (a run is where ticks get paid for). The journal's charts
are GETs and must never spend, so they pass ``allow_fetch=False`` and read
through the cache-only doors ``ticks`` already keeps for exactly this
(``cached_rth`` / ``cached_overnight`` / ``cached_post``, and
``contract_for_cached`` for the roll). A session that was never bought draws as
"no ticks", not as a purchase.

THE ROLL IS NOT THE LABEL. A journal trade's ``instrument`` is whatever ATAS
stamped on the export, which is the front month *at export time* — every 2026
export says `NQU6@CME`, including the January trades that were NQH6. Resolving a
session by that label finds the wrong contract's ticks or none at all. The day
decides the contract, through the roll map; the caller passes a root and this
resolves it.
"""
...
def _epoch_local(ts_utc, tz) -> pd.Series | int:
    ...
    Lives here rather than in ``charts_data`` because both payload builders need
    it and this is the lower layer — ``charts_data`` imports it back. Dropping
    the zone is deliberate and load-bearing for the chart: lightweight-charts
    plots a bare number, so weekend and overnight gaps collapse natively and
    09:30 reads as 09:30 whatever the session's offset was.
    """
...
#: Bar size for a tick-bar chart with no engine behind it. The Lab's Strategies
#: charts draw the run's own ``ticks_per_bar``; the research benches and the
#: journal have no engine to inherit one from, and 500 is what Interactions and
#: Drafts already standardised on.
DEFAULT_TICKS_PER_BAR = 500
...
def _footprint(t: pd.DataFrame, b: pd.DataFrame, ts_size: float) -> list[list[list[float]]]:
    ...
    an approximation of a distribution the tape already holds exactly, and a
    groupby is the whole difference. Shipping it per bar rather than
...
def vwap_slots(gx_rows: list[dict], ny_rows: list[dict], wk_rows: list[dict],
    ...
    A journal chart has no engine, so nothing was "traded" — it passes ``"ny"``,
    the session anchor the RTH day is read against.
    """
    return {
        "vwap_globex": gx_rows,
        "vwap_ny": ny_rows,
...
class SessionFrame:
    ...
    ``ticks`` is the tick frame every index below points into (night + RTH +
    the recovered post hour, whichever of those exist), and ``bar_time`` snaps an
    instant onto the drawn bar grid — the two things a caller needs to lay its own
    markers over the session.
    """
    ticks: pd.DataFrame
    bars: list[dict]
    vwap_globex: list[dict]
    vwap_ny: list[dict]
...
def resolve_symbol(contract: str, day: date, allow_fetch: bool = True) -> str | None:
    """Which raw contract this session actually traded.

    ``contract`` may be a root (`NQ`), a journal instrument (`NQU6@CME`) or an
    exact contract. A root rolls; anything else is taken at face value by
    ``ticks.rolls`` — which is right for a run that pinned a contract and wrong
    for a journal row, whose label is the front month at *export* time. So an
    instrument is reduced to its root first and then rolled by the day.
    ...
    if not tickmod.rolls(root):
        return contract
    if allow_fetch:
        return tickmod.contract_for(root, day)
    return tickmod.contract_for_cached(root, day)


def _session_ticks(sym: str, day: date, overnight: bool, allow_fetch: bool):
    """(session ticks, night) for one day — buying only when allowed.

    ``get_day_ticks`` buys what it doesn't find, which is right for a run and
    wrong for a GET. The cache-only path rebuilds the same two shapes from the
    doors ``ticks`` already keeps for charts: night+RTH when the caller wants the
    night in the traded frame, RTH alone otherwise.
    """
    if allow_fetch:
...
def session_frame(
    *,
    ...
    Every chart shows the same thing: the overnight candles from 18:00 ET, the
    RTH session, both anchored VWAPs, and both developing profiles. What differs
    between callers is only which of those a strategy *read* — and the payload
    says so (``vwap_anchor``) rather than hiding a layer.
    ...
    built in two pieces, not one stream —
      ...
      * the caller's own bars, from its own tick frame (RTH-anchored for a
    ...
    if sym is None:
        return None
    t = _session_ticks(sym, day, overnight, allow_fetch)
    if t is None or t.empty:
        return None
    b = barmod.tick_bars(t, ticks_per_bar)  # the engine's bars, from its own ticks
    if b.empty:
        return None
    ...
    if overnight:
        ...
        if on is None or on.empty:
            ...
            rth_i0 = len(on)
            lead = _lead_bars(on, ticks_per_bar)
    ...
    if post is not None and not post.empty:
        if resolution == "tick":
            b_all = pd.concat([b_all, _post_bars(post, ticks_per_bar, rth_end)],
                              ignore_index=True)
    ...
    # restarts at the bell. Both are computed for every chart — the layer is the
    # same on every chart, and the caller's config (not the picture) says which
    # anchor a rule read. Each result indexes straight into `b_all`/`times`, so a
    # bar with no value area yet is dropped by the NaN filter rather than
    # mis-drawn: the overnight candles on the NY anchor (their shifted end is
    # negative, so nothing accumulates), and — when the night is absent — the
    # Globex anchor entirely.
    tsz = tick_size(contract)
    profile_gx_rows = (
    ...
    return SessionFrame(
        ticks=full,
        bars=bars_rows,
        vwap_globex=vwap_gx_rows,
        vwap_ny=vwap_ny_rows,
