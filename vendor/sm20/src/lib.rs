...
///
/// VENDOR PATCH (atas_journal): the serde derives are ours. Upstream serialises
/// this struct field-by-field at its one call site; we round-trip the whole
/// thing through a subprocess, so it has to derive. Every field already did.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
mod tests {
    ...
    fn arena_preview_is_deterministic_complete_and_non_mutating() {
        ...
        let fixture: serde_json::Value =
            serde_json::from_str(include_str!("../sm20ArenaParityFixture.json"))
