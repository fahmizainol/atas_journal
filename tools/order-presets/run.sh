#!/usr/bin/env bash
# Regenerate the order-preset fixture from the real `lib/orderPresets.ts` and
# the ruler it reads (`lib/volRuler.ts`).
#
#   tools/order-presets/run.sh
#
...
# tests/fixtures/order_presets.json. Commit the fixture with whatever change to
# `orderPresets.ts` moved it — an uncommitted regeneration is how the alarm gets
...
node "$tmp/gen.mjs" > "$root/tests/fixtures/order_presets.json"
echo "wrote tests/fixtures/order_presets.json"
