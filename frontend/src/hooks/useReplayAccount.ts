// The replay account, as the browser reads it.
//
// Everything here is derived on the server from the attempts on disk (see
// `journal.replay_account`) — there is no balance to keep in sync and nothing
// to post back. What this file adds is the one thing the server cannot do:
// count down.
//
...
export interface AccountView {
   ...
   *  makes it safe to compare a live equity against in here. */
  floor: number;
  peak_close: number;
  status: "live" | "blown" | "cooldown" | "can_reset";
  day_net: number;
